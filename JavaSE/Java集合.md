...
- **HashMap**
  ...
  - 底层实现原理
    ...
    3. 添加元素的过程
       ...
       - 首先计算entry1中 key (所在类的 hashCode( ) 返回) 的哈希值，将其高16位异或低十六位得到最终的哈希值，得到在底层Entry[ ] 中要存储的位置 i
         ...
         - 如果位置 i 上已经存在一个或多个 entry / 发生冲突 ，通过循环的方法，依次比较 entry1 的key 和其他的 entry 的 key
           ...
           - 如果 hash 相同，比较 entey1 的key 
             - 如果遍历一遍后，key 不同，则 entry1 添加成功。
             - 如果 key 相同 value 不同，则让 entry1 的 value，替换 value.equals ( ) 为 true 的 entry 的 value，
             - 如果 key 相同 value 相同，添加失败
    ...
    4. JDK1.8相较于之前的变化：
       ...
       - 形成链表结构时，新添加的 key-value 对在链表的尾部（七上头插，八下尾插）
